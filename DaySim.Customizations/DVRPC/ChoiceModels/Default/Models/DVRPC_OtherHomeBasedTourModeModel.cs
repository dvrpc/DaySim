...
namespace DaySim.ChoiceModels.Default.Models {
  class DVRPC_OtherHomeBasedTourModeModel : OtherHomeBasedTourModeModel {
    protected override void RegionSpecificCustomizations(ChoiceProbabilityCalculator.Alternative alternative, ITourWrapper tour, int pathType, int mode, IParcelWrapper destinationParcel) {
      ...
      int coreCBDOrig = (tour.OriginParcel.District == 1 && tour.OriginParcel.ZoneKey < 5000).ToFlag();// (tour.OriginParcel.HouseholdsBuffer1 + tour.OriginParcel.EmploymentTotalBuffer1 >= 31000).ToFlag();
      int coreCBDDest = (destinationParcel.District == 1 && destinationParcel.ZoneKey < 5000).ToFlag();// (destinationParcel.HouseholdsBuffer1 + destinationParcel.EmploymentTotalBuffer1 >= 31000).ToFlag();
